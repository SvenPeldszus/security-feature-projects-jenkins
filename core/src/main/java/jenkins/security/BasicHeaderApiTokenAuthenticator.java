...
public class BasicHeaderApiTokenAuthenticator extends BasicHeaderAuthenticator {
     ...
     * {@link SecurityListener#fireFailedToAuthenticate(String)} because it will be done in the
     * {@link BasicHeaderRealPasswordAuthenticator} in the case the password is not valid either
     */
    @Override
    @Secrecy
    @Integrity
    public Authentication authenticate2(HttpServletRequest req, HttpServletResponse rsp, String username, String password) throws ServletException {
        ...
        if (u != null) {
            ...
            try {
                ...
                // The token was valid, but the impersonation failed. This token is clearly not his
                // real password,
                // so there's no point in continuing the request processing. Report this error and
                // abort.
